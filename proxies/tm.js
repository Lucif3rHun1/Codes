...
{
    const proxies = processProxyFile('proxies.csv')
    let randomIndex = Math.floor(Math.random() * proxies.length);
    let proxy = proxies[randomIndex]
    let proxyOptions = proxy ? parseProxyString(proxy) : undefined;
    let agent = proxy ? new HttpsProxyAgent(`http://${proxyOptions.username}:${proxyOptions.password}@${proxyOptions.host}:${proxyOptions.port}`) : undefined;
    let repeatedString = '-'.repeat(4096);
    console.log(`http://${proxyOptions.username}:${proxyOptions.password}@${proxyOptions.host}:${proxyOptions.port}`)
    try {
